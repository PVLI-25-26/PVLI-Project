...
import { BossMovementControllerComponent } from "../../components/BossMovementController.js";
...
/**
 * Slow tank that reacts to damage taken by allies.
 ...
 */
export class Boss extends BillBoard {
    /**
    ...
    constructor(scene, x, y, id, config) {
        ...
        console.log("Created Boss Enemy:", this.id);
    ...
    addComponents() {
        ...
        EventBus.emit('bossHealthInitialized', 
            { enemy: this, maxHP: this.config.maxHP });

        // Add BossMovementControllerComponent// The patrol route isn't a TiledProperty, it is set in the factory
        const controller = new BossMovementControllerComponent(this, getCustomTiledProperty(this.config, "state"), this.config.patrolRoute);
    ...
    destroy() {
        EventBus.emit('bossKilled', { enemy: this });
